...
public class PolyNavAgent : MonoBehaviour{
    ...
    public MapDir moveDir= MapDir.center;
	...
	void LateUpdate(){
		...
		if (rotateTransform){
            ...
            moveDir = GetActorDir(movingDirection.x, movingDirection.y);
            PlayAnimation(velocity != Vector2.zero);
    ...
    private string clip_name_older;
    void PlayAnimation(bool run)
    {
        if(m_Animator == null)
            return;
        string clip_name = SkillManager.idle_shang;
        switch(moveDir)
        {
            case MapDir.center: clip_name = run ? SkillManager.run_shang : SkillManager.idle_shang; break;
            case MapDir.left: clip_name = run ? SkillManager.run_zuo : SkillManager.idle_zuo; break;
            case MapDir.right: clip_name = run ? SkillManager.run_you : SkillManager.idle_you; break;
            case MapDir.top: clip_name = run ? SkillManager.run_shang : SkillManager.idle_shang; break;
            case MapDir.down: clip_name = run ? SkillManager.run_xia : SkillManager.idle_xia; break;
            case MapDir.lefttop: clip_name = run ? SkillManager.run_zuoshang : SkillManager.idle_zuoshang; break;
            case MapDir.leftdown: clip_name = run ? SkillManager.run_zuoxia : SkillManager.idle_zuoxia; break;
            case MapDir.righttop: clip_name = run ? SkillManager.run_youshang : SkillManager.idle_youshang; break;
            case MapDir.rightdown: clip_name = run ? SkillManager.run_youxia : SkillManager.idle_youxia; break;
        }
        if(!clip_name.Equals(clip_name_older))
        {
            m_Animator.CrossFadeInFixedTime(clip_name, 0.1f);
            clip_name_older = clip_name;
        }
    }
    ...
    /// <returns></returns>
    MapDir GetActorDir(float dirx, float diry)
    {
        int dir = 0;
        float offset = 0.3f;
        //判断x方位
